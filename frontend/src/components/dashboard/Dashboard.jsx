...
const mapStatusForDisplay = (backendStatus) => {
  ...
  if (backendStatus === "compliant" || backendStatus === "done")
    return "compliant";
...
const Dial = ({ status, size = 28 }) => {
  ...
  return (
    ...
    >
      ...
      <path
        d={`M ${center + radius * Math.cos((-120 * Math.PI) / 180)} ${center + radius * Math.sin((-120 * Math.PI) / 180)} A ${radius} ${radius} 0 0 1 ${center + radius * Math.cos((-60 * Math.PI) / 180)} ${center + radius * Math.sin((-60 * Math.PI) / 180)}`}
        fill="none"
        stroke="#3F6B52"
      ...
      />
      <path
        d={`M ${center + radius * Math.cos((-20 * Math.PI) / 180)} ${center + radius * Math.sin((-20 * Math.PI) / 180)} A ${radius} ${radius} 0 0 1 ${center + radius * Math.cos((20 * Math.PI) / 180)} ${center + radius * Math.sin((20 * Math.PI) / 180)}`}
        fill="none"
        stroke="#C98A1E"
        strokeWidth={strokeWidth * 0.5}
        opacity="0.6"
      />
      <path
        d={`M ${center + radius * Math.cos((60 * Math.PI) / 180)} ${center + radius * Math.sin((60 * Math.PI) / 180)} A ${radius} ${radius} 0 0 1 ${center + radius * Math.cos((120 * Math.PI) / 180)} ${center + radius * Math.sin((120 * Math.PI) / 180)}`}
        fill="none"
        stroke="#A23E2A"
        strokeWidth={strokeWidth * 0.5}
        opacity="0.6"
...
const ComplianceDashboard = ({ configData }) => {
  ...
  const CATEGORIES = useMemo(() => {
    ...
    requirements.forEach((r) => {
      if (!seen.has(r.category)) seen.set(r.category, r.category);
    });
  ...
  const handleRequirementUpdate = async (id, updates) => {
    ...
    const completedChanged =
      updates.lastCompleted && updates.lastCompleted !== current?.lastCompleted;
  ...
  const categoryStatus = (catKey) => {
    ...
    if (items.length > 0 && items.every((r) => r.status === "compliant"))
      return "compliant";
    return "";
  };
  ...
  return (
    <div className="app-container">
      ...
      <aside className="sidebar">
        ...
        <div className="operator-section">
          ...
          <div className="operator-name">
            {configData?.operatorName || "Yunoya LTD"}
          </div>
        </div>
      ...
      <main className="main-content">
        {loading && (
          <p style={{ padding: 24 }}>Loading your compliance calendar…</p>
        )}
        {loadError && (
          <p style={{ padding: 24, color: "#c0392b" }}>{loadError}</p>
        )}
            ...
            <>
                  ...
                  {CATEGORIES.map((cat, idx) => (
                    <React.Fragment key={cat.key}>
                      ...
                      {idx < CATEGORIES.length - 1 && (
                        <span className="spine-connector"></span>
                      )}
                    </React.Fragment>
                  ))}
                </div>
              ...
              <div className="dashboard-grid">
                        ...
                        return (
                          <React.Fragment key={cat.key}>
                              ...
                              <td colSpan="6" className="category-label-cell">
                                ...
                                {isOpen && (
                                  <div className="category-breakdown-dropdown category-breakdown-dropdown--floating">
                                    <div
                                      className={`cat-count-row cat-count-row--compliant ${counts.compliant === 0 ? "is-zero" : ""}`}
                                    >
                                      <span className="cat-count-dot"></span>
                                      <span className="cat-count-name">
                                        Compliant
                                      </span>
                                      <span className="cat-count-value">
                                        {counts.compliant}
                                      </span>
                                    </div>
                                    <div
                                      className={`cat-count-row cat-count-row--due ${counts.due === 0 ? "is-zero" : ""}`}
                                    >
                                      <span className="cat-count-dot"></span>
                                      <span className="cat-count-name">
                                        Due
                                    ...
                                    <div
                                      className={`cat-count-row cat-count-row--pastdue ${counts.pastDue === 0 ? "is-zero" : ""}`}
                                    >
                                      <span className="cat-count-dot"></span>
                                      <span className="cat-count-name">
                                        Past Due
                                    ...
                                    <div
                                      className={`cat-count-row cat-count-row--unset ${counts.unset === 0 ? "is-zero" : ""}`}
                                    >
                                      <span className="cat-count-dot"></span>
                                      <span className="cat-count-name">
                                        Not Set
                            ...
                            {items.map((req) => (
                              ...
                              >
                                <td className="td-dial">
                                  <Dial status={req.status} size={26} />
                                </td>
                                <td className="td-citation">{req.citation}</td>
                                <td>
                                  {req.description}
                                  {req.requiresOperatorInput &&
                                    !req.frequencyValue && (
                                      <span
                                        style={{
                                          marginLeft: 8,
                                ...
                                <td className="td-due">
                                  {req.nextDue
                                    ? formatDate(req.nextDue)
                                    : "Not set"}
                                </td>
                                <td className="td-assign">
                                  {req.assigned || "Unassigned"}
                                </td>
                ...
                <div className="escalation-rail">
                  ...
                  >
                    Wire this list to GET /api/contacts (sorted by
                    escalationLevel) as a follow-up - it's currently still the
                    placeholder names from the original UI.
                  </p>
                </div>
              </div>
            </>
